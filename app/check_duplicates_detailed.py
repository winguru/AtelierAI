...
print(
    "=== Checking for true duplicates (same authority_id, normalized_external_name) ===\n"
)
...
if not duplicates:
    ...
    for dup in duplicates:
        print(
            f"Authority {dup['authority_id']}: '{dup['normalized_external_name']}' has {dup['count']} records"
        )
        # Show all records
        detail_query = """
        ...
        for detail in conn.execute(
            detail_query, (dup["authority_id"], dup["normalized_external_name"])
        ):
            print(
                f"  ID {detail['id']}: tag_id='{detail['external_tag_id']}', name='{detail['external_name']}'"
            )
        print()
...
for row in conn.execute(query):
    print(
        f"  ID {row['id']}: tag_id='{row['external_tag_id']}' | name='{row['external_name']}' | norm='{row['normalized_external_name']}'"
    )
