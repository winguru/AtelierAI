    ...
    def _debug_session_token(self, debug: bool) -> None:
        ...
        if debug:
            if not self.api.session_cookie or len(self.api.session_cookie) < 100:
                print(
                    f"  DEBUG: ⚠️  Session token invalid! Length: {len(self.api.session_cookie) if self.api.session_cookie else 0}"
                )
            else:
                print(
                    f"  DEBUG: ✅ Session token valid. Length: {len(self.api.session_cookie)}"
                )
