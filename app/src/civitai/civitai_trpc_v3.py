#!/usr/bin/env python3
"""
CivitAI tRPC Client - with browsing preferences and pagination

Version 1.3 - Supports proper pagination with cursor-based fetching.
"""
...
class CivitaiTrpcClient:
    ...
    def __init__(
        ...
        :param verbose: If True, print request URLs for debugging.
        :param very_verbose: If True, print even more request info and response data for debugging.
    ...
    def _fetch_with_pagination_limit(self, limit: Optional[int]) -> bool:
        """
        ...
        Behavior:
            - -1: Use pagination (fetch ALL until no cursor)
            - None or 1-50: Single page only (fetch limit items)
            - >50: Use pagination to fetch multiple pages
        """
        # -1 means fetch ALL (use pagination until no cursor)
        if limit == -1:
            return True
        # None or 1-50 means single page (no pagination)
        if limit is None or (limit is not None and limit <= 50):
            return False
        # >50 means use pagination
    ...
    def _fetch_infinite_images_page(self, payload: Dict[str, Any]) -> Dict[str, Any]:
        """Fetch a single page of infinite images."""
        if self.verbose:
            print(
                f"🔍 DEBUG: Fetching page with cursor: {payload.get('cursor', None)}"
            )
        ...
        if self.verbose:
            print(f"🔍 DEBUG: Response items count: {len(items)}")
            print(f"🔍 DEBUG: Response nextCursor: {response.get('nextCursor')}")
    ...
    def _handle_page_limit(
        ...
        if total_items_fetched >= item_limit:
            excess_items = total_items_fetched - item_limit
            if excess_items > 0:
                all_items = all_items[:-excess_items]
                if self.verbose:
                    print(
                        f"✅ Reached limit of {item_limit} items (fetched {total_items_fetched - excess_items} + {excess_items} on last page)"
                    )
    ...
    def _paginate_infinite_images(
        ...
        while not should_break:  # Loop until limit reached or no more cursors
            if current_cursor:
            ...
            should_break, all_items = self._handle_page_limit(all_items, total_items_fetched, limit)
            if should_break:
                if self.verbose:
                    print(
                        f"✅ Reached limit of {limit} items (fetched {total_items_fetched} total)"
                    )
                break
            # Check for next page
            current_cursor = response.get("nextCursor")
            ...
            should_break, all_items = self._handle_page_limit(
                all_items, total_items_fetched, limit
            )
            if should_break:
                break
        ...
        return {
            ...
            "totalFetched": total_items_fetched,
    ...
    def get_infinite_images(
        ...
        **other_params: Any,
        ...
        # Check if we should use pagination loop
        use_pagination = self._fetch_with_pagination_limit(limit)

        if self.verbose:
            mode = (
                "UNLIMITED (∞) items"
                if limit == -1
                else f"LIMITED to {limit} items" if limit else "DEFAULT (50) items (one page)"
            )
            print(f"📊 Pagination mode: {mode}")

        if not use_pagination:
            # Fetch just one page
            return self._fetch_infinite_images_page(payload)

        # Determine the actual limit for the loop
...
if __name__ == "__main__":
    ...
    print("🚀 CivitAI tRPC Client v1.3 (with pagination fix)")
    print("=" * 70)
...
Examples:
  # Basic usage
  python civitai_trpc_v3.py

  # Auto-load browsing settings from API
  python civitai_trpc_v3.py --auto-load-settings
  python civitai_trpc_v3.py -a

  # Verbose mode (show request URLs and response data)
  python civitai_trpc_v3.py --verbose
  python civitai_trpc_v3.py -v

  # Limit number of images returned
  python civitai_trpc_v3.py --limit 10
  python civitai_trpc_v3.py -l 10

  # Fetch all items (use pagination)
  python civitai_trpc_v3.py --limit -1

  # Fetch 150 items (3 pages with pagination)
  python civitai_trpc_v3.py --limit 150
  python civitai_trpc_v3.py -l 150

  # Combine options
  python civitai_trpc_v3.py -v --auto-load-settings --limit 5
        """,
    )
    parser.add_argument(
        "-v",
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Limit number of images to fetch (default: 50, one page)",
    )
    ...
    try:
        ...
        # 3. Get Infinite Images with pagination
        ...
        if args.limit >= 0:
            ...
            print("📊 Limit: 50 items (default, one page)")
        ...
        total_fetched = images_data.get("totalFetched", len(items))
        pages_fetched = images_data.get("pagesFetched", 1)

        print(f"✅ Found {len(items)} images")
        if pages_fetched > 1:
            print(f"   Fetched from {pages_fetched} pages")
            print(f"   Total API calls made: {pages_fetched}")

        # Show last image details as example
        if items:
            last_img = items[-1]
            print(f"\n📸 Example (last) image:")
            print(f"   Item #: {len(items)}")  # Show the overall item number based on total fetched
            print(f"   ID: {last_img.get('id')}")
            print(f"   URL: https://civitai.com/images/{last_img.get('id')}")
            print(f"   Name: {last_img.get('name')}")
            nsfwLevel = last_img.get("nsfwLevel", 0)
            print(f"   NSFW Level: {nsfwLevel}")
            explanation = client._explain_browsing_level(nsfwLevel)
            print(
                f"   NSFW Flags: {', '.join([cat for cat, enabled in explanation.items() if enabled])}"
            )
            print(f"   Width: {last_img.get('width')}")
            print(f"   Height: {last_img.get('height')}")

        # Example of how to handle pagination
        next_cursor = images_data.get("nextCursor")
        if next_cursor:
            print(f"\n📄 Next cursor available: {next_cursor}")
            print("   To get the next page, call:")
            print(
                f"      client.get_infinite_images(collection_id={collection_id}, cursor=next_cursor)"
            )
        else:
            print("\n✅ All items fetched (no more pages available)")
