...
- Archive links for deleted models (civitaiarchive.com)

**Features from v2.0:**
- Uses `CivitaiAPI` singleton for API calls
- Fetches tags from `tag.getVotableTags` endpoint
- Includes "Top Tags" section in analysis report
...
#### **Tags** (NEW)
- Most common tags across collection
- Tag usage frequency and percentage
- Sorted by relevance score
- Helps understand collection themes and subjects
...
- **Tags fetched and displayed** for each image
...
# See: Common concepts, dominant styles, recurring subjects, tags
```
...
The analyzer now fetches tags using the `tag.getVotableTags` API endpoint:
- Tags are fetched per image during scraping
- Tags are sorted by relevance score (highest first)
- Tag occurrences are aggregated across the collection
- "Top Tags" section shows most common tags with percentages

## Integration with Other Tools

The analyzer uses:
- **CivitaiAPI** - For fetching collection data and tags (NEW)
- **CivitaiPrivateScraper** - Legacy scraper (still supported)
